...
void enablePosControl(ODriveCAN &odrv, const char* odrv_name) {
  odrv.setControllerMode(ODriveControlMode::CONTROL_MODE_POSITION_CONTROL, ODriveInputMode::INPUT_MODE_POS_FILTER);
}
...
void iniciarOdrive(){
  ...
  // Serial.println("attempting to read bus voltage");
  // Get_Bus_Voltage_Current_msg_t vbus;
  // if (!odrv1.request(vbus, 1000)) {
  //   Serial.println("vbus request failed!");
  //   while (true); // spin indefinitely
  // }
  // Serial.print("DC voltage Odrive [V]: ");
  // Serial.println(vbus.Bus_Voltage);

  Serial.println("Activando ODrives...");
  ...
  enableClosedLoopControl(odrv1, odrv1_user_data, "ODrive1");
  enableClosedLoopControl(odrv3, odrv3_user_data, "ODrive3");

  odrv0.setPosition(-altura);
  odrv2.setPosition(altura);

  // Serial.println("Odrives activados!!!");
}
